...
router.delete('/:id', async (req, res) => {
  ...
  try {
    await pool.query('DELETE FROM maquinario WHERE id = $1', [id]);
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT * FROM maquinario WHERE id = $1', [id]);
...
router.put('/:id', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'UPDATE maquinario SET nome = $1, dataaquisicao = $2 WHERE id = $3 RETURNING *',
      [nome, aquisicao, id]
    );
